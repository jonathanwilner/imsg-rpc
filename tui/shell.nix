...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    export IMSG_TUI_IMSG_BIN=''${IMSG_TUI_IMSG_BIN:-imsg}
    imsg-tui-run() {
      cargo build --release >/dev/null
      echo "Starting imsg-tui (transport=$IMSG_TUI_TRANSPORT host=$IMSG_TUI_HOST port=$IMSG_TUI_PORT)"
      exec ./target/release/imsg-tui --transport "$IMSG_TUI_TRANSPORT" --host "$IMSG_TUI_HOST" --port "$IMSG_TUI_PORT" --imsg-bin "$IMSG_TUI_IMSG_BIN"
    }
    export -f imsg-tui-run
  '';
}
